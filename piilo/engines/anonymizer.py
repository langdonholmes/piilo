...
class SurrogateAnonymizer(AnonymizerEngine):
    ...
    def anonymize(self, text: str, analyzer_results: List[RecognizerResult]):
        ...
        operators = self._AnonymizerEngine__check_or_add_default_operator(
            {
                "STUDENT": OperatorConfig(
                    "custom", {"lambda": self.generate_surrogate}
                ),
                "EMAIL_ADDRESS": OperatorConfig(
                    "replace", {"new_value": "janedoe@aol.com"}
                ),
                "PHONE_NUMBER": OperatorConfig(
                    "replace", {"new_value": "888-888-8888"}
                ),
                "URL": OperatorConfig("replace", {"new_value": "aol.com"}),
            }
        )

        res = self._operate(text, analyzer_results, operators, OperatorType.Anonymize)

...
class KaggleThirdAnalyzer(LocalRecognizer):
    ...
    def __init__(self):
        ...
        self.names_bl = pd.concat(
            [self.first_names_bl["first_name"], self.last_names_bl["last_name"]]
        ...
        self.first_name_full_dict = self.first_names_bl.set_index("first_name")[
            "count"
        ].to_dict()
        self.first_name_high_dict = self.first_name_high.set_index("first_name")[
            "count"
        ].to_dict()
        self.first_name_low_dict = self.first_name_low.set_index("first_name")[
            "count"
        ].to_dict()
        self.last_name_full_dict = self.last_names_bl.set_index("last_name")[
            "count"
        ].to_dict()
        self.last_name_high_dict = self.last_name_high.set_index("last_name")[
            "count"
        ].to_dict()
        self.last_name_low_dict = self.last_name_low.set_index("last_name")[
            "count"
        ].to_dict()
        ...
        self.words_10k_dict = dict(
            zip(
                self.words_10k.iloc[:, 0].to_list(),
                range(self.words_10k.shape[0], 0, -1),
            )
        )
        self.words_1k_dict = dict(
            zip(
                self.words_1k.iloc[:, 0].to_list(), range(self.words_1k.shape[0], 0, -1)
            )
        )
    ...
    def token_pass(
        ...
        for ix, token in enumerate(tokens):
            ...
            # ID rule
            if len(text) > 8 and (
                text.isdigit() or (not text[:2].isdigit() and text[-4:].isdigit())
            ):
            ...
            elif ("http" in text or "www" in text) and all(
                ext not in text for ext in self.url_extensions_bl
            ):
            ...
            for delimiter in self.phone_delimiters:
                ...
                if len(phone_number_parts) == 2 and all(
                    part.isdigit() and len(part) > 2 for part in phone_number_parts
                ):
            ...
            if (
                len(phone_number_parts) == 3
                and len(phone_number_parts[0]) == 3
                and len(phone_number_parts[1]) == 3
                and len(phone_number_parts[2]) == 4
            ):
                res = self.create_result("PHONE_NUMBER", token)
    ...
    def generate_features(self, target_strings: List[str]) -> List[Union[int, bool]]:
        """
        ...
        for target_string in target_strings:
            ...
            conditions = [
                ...
                target_string
                in self.last_name_low_dict,  # This was repeated for some reason; probably a mistake
            ...
            string_feature_list.extend(
                [
                    i.get(target_string.lower(), 0)
                    for i in [self.words_10k_dict, self.words_1k_dict]
                ]
            )
            string_feature_list.extend(
                [
                    i.get(target_string, 0)
                    for i in [
                        self.first_name_full_dict,
    ...
    def load_models(self) -> List[xgb.XGBClassifier]:
        ...
        if models_splitter == [] or models_fp_remove == []:
            logger.error(
                "XGB models not found. Make sure to download them from the Kaggle competition page: https://www.kaggle.com/code/devinanzelmo/piidd-efficiency-3rd-inference/input?select=xgb_final"
            )
            raise FileNotFoundError(
                r"XGB models not found. Make sure to download them from the Kaggle competition page: https://www.kaggle.com/code/devinanzelmo/piidd-efficiency-3rd-inference/input?select=xgb_final"
            )
    ...
    def make_predictions(
        self, models: List[xgb.XGBClassifier], feat: np.ndarray[Union[int, bool]]
    ) -> np.array:
        """
    ...
    def feature_pass(
        self, results: List[RecognizerResult], feature_predictions: np.array
    ...
    def analyze(
        ...
        name_indices, padded_data = self.generate_padded_name_tokens(
        ...
        results, padded_names, padded_names_pos = self.token_pass(
        ...
        feature_predictions = self.make_predictions(
            models_fp_remove, concatenated_features
        )
        feature_threshold = self.cfg["thresholds"]["feature"]
        feature_predictions = (feature_predictions[:, 1] > feature_threshold).astype(
            np.int32
        )
        ...
        for result in results:
            ...
            if to_keep:
                for filtered in filtered_results:
                    ...
                    if result.contained_in(filtered) or result.intersects(filtered):
                        to_keep = False
                        break

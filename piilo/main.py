...
def anonymize_batch(dir: str, entities=None, language="en", file_format="csv") -> None:
    ...
    if file_format not in file_formats:
        raise ValueError(
            f"File format {file_format} not supported. Please choose from {file_formats}"
        )
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Directory {dir} not found. Please enter a valid directory."
        )
    ...
    for file in target_files:
        try:
            ...
            logger.error(
                f"File {file} could not be anonymized due to the following error: {e}"
            )
            print(
                f"File {file} could not be anonymized due to the following error: {e}"
            )
...
def anonymize_batch_cli():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing text files to anonymize; defaults to the current directory",
    )
    parser.add_argument(
        "--entities", type=str, nargs="*", default=None, help="Entities to anonymize"
    )
    parser.add_argument(
        "--language",
        type=str,
        default="en",
        help="Language of the text files; currently only supports 'en'",
    )
    parser.add_argument(
        "--file_format",
        type=str,
        choices=["csv", "txt"],
        default="csv",
        help="Output file format; currently supports 'csv' and 'txt'",
    )
    ...
    if args.dir is None:
        print(
            f"""Anonymizing text files in the current directory: {os.getcwd()}. If this is not what you want,
